...
// --- Simplificado: ya no hay botones ni almacenamiento de tema ---
// La preferencia se aplica en el IIFE superior que usa prefers-color-scheme.
// Aquí solo inicializamos las animaciones de separadores (si corresponde).
document.addEventListener('DOMContentLoaded', () => {
  try {
    // Respetar preferencia de reducir movimiento
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (!reduceMotion) {
      const logos = document.querySelectorAll('.section-separator .separator-logo');
      logos.forEach((logo, idx) => {
        gsap.set(logo, { transformOrigin: 'center center' });
        gsap.fromTo(logo, { scale: 0.9 }, { scale: 1.12, duration: 1.0, repeat: -1, yoyo: true, ease: 'sine.inOut', delay: idx * 0.12 });
      });
    }
  } catch (e) {
